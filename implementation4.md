...
## Phase 12 — Vision: Photo-Based Reasoning
**Owner:** Dev B | **Effort:** ~4-5 hrs

**Goal:** Give the companion eyes. This is the strongest replacement for the old
"railway/off-route station" scenario you inherited from the original concept — instead of
faking transit data, an elder who's disoriented can literally show the companion what
they're looking at, and it reasons about it using GPT-5.6's multimodal capability. This
directly answers your original problem framing: some people genuinely cannot describe
where they are in words, but they can point a camera.

Tasks:
- Add a camera/photo icon on the patient companion screen, alongside the existing mic icon
  from Phase 8.4
- For demo reliability, prepare **2-3 pre-set demo images** (e.g. an unfamiliar street/
  station scene, a medication bottle, a familiar home interior) rather than relying on live
  camera capture during recording — same principle as the voice fallback in Phase 10. A
  live upload option is fine to also support, but don't depend on it working perfectly on
  camera
- Send the image (plus any accompanying text, e.g. "where am I?") to a GPT-5.6 multimodal
  call **using the same companion persona and system prompt established in Phase 8.1-8.2**
  — the response must come back in the same warm, human voice as text/voice replies, not
  a cold, separate "image analysis" report bolted onto the side
- Ground the response in the actual image content plus existing dashboard context (known
  routine, safe-zone location) — e.g. "this doesn't look like your usual street near
  Greenfield" only if that's actually inferable, not asserted generically
- If the image + accompanying conversation suggests genuine disorientation, reuse the
  **exact same `escalate_to_caregiver` tool from Phase 8.3** — do not build a parallel
  vision-specific escalation path. One consistent decision system, now with one more input
  type available to it
- A benign photo (e.g. "what is this pill") should be answered helpfully without
  triggering any escalation — vision input needs the same restraint as text input

**Verify before proceeding:**
- [ ] Companion correctly and specifically describes/reasons about at least 2 distinct
      demo images (a location-type photo and an object/medication-type photo) — not a
      generic "I see an image" response
- [ ] A location photo combined with a verbal "I don't know where I am" correctly invokes
      `escalate_to_caregiver`, with reasoning that references the actual image content
- [ ] A benign object photo does NOT falsely trigger caregiver escalation
- [ ] Vision responses are indistinguishable in tone from text/voice responses — same
      companion voice, not a separate clinical-sounding analysis mode
- [ ] The pre-set demo images work reliably every time you test them, since these are what
      you'll actually use in the recorded video
...
OpenAI hackathon — it's a concrete, checkable demonstration of GPT-5.6's actual multimodal
capability, not just text completions with better prompting, and it directly upgrades the
weakest inherited piece of your original concept (the faked railway subsystem) into
something genuinely useful: an elder who can't describe where they are can still show you.
The daily journal (Phase 13) is cheap but disproportionately effective for **Quality of
Idea** — it reframes the product from "an alert system" to "something that actually thinks
about my parent's whole day," which is a more emotionally resonant pitch in your demo video
than any individual feature. Together, prioritize Phase 12 first if you have to choose —
